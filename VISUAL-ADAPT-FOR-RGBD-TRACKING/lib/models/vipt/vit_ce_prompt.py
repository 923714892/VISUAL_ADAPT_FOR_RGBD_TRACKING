...
from .prompt_cross import prompt_embed
...
class VisionTransformerCE(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.pos_embed_z_prompt = nn.Parameter(torch.zeros(1, self.num_patches_template, embed_dim))
        self.pos_embed_x_prompt = nn.Parameter(torch.zeros(1, self.num_patches_search, embed_dim))

        self.prompt_type = prompt_type
        # various architecture
        if self.prompt_type in ['vipt_shaw', 'vipt_deep_no_filter']:
            prompt_blocks = []
            block_nums = depth if self.prompt_type == 'vipt_deep_no_filter' else 1
            # block_nums = 1
            for i in range(block_nums):
                prompt_blocks.append(prompt_embed(dim= embed_dim))
            ...
            # self.prompt_embed = prompt_embed(dim= embed_dim)
    ...
    def forward_features(self, z, x, mask_z=None, mask_x=None,
        ...
        # if self.prompt_type in ['vipt_shaw', 'vipt_deep_no_filter']:
        #     z_feat = self.prompt_norms[0](z)
        #     x_feat = self.prompt_norms[0](x)
        #     z_dte_feat =self.prompt_norms[0](z_dte)
        #     x_dte_feat = self.prompt_norms[0](x_dte)
        #     x_f = torch.cat((z_feat,x_feat),dim=1)
        #     prompt_emb, z_dte, x_dte = self.prompt_blocks[0](x_f,z_dte_feat,x_dte_feat)
        ...
        z += self.pos_embed_z
        x += self.pos_embed_x
        z_dte += self.pos_embed_z_prompt
        x_dte += self.pos_embed_x_prompt
        ...
        x = self.pos_drop(x)
        z_dte = self.pos_drop(z_dte)
        x_dte = self.pos_drop(x_dte)

        '''input prompt: by adding to rgb tokens'''
        if self.prompt_type in ['vipt_shaw', 'vipt_deep_no_filter']:
            x_feat = self.prompt_norms[0](x)
            z_dte = self.prompt_norms[0](z_dte)
            x_dte = self.prompt_norms[0](x_dte)
            prompt_emb, z_dte, x_dte = self.prompt_blocks[0](x_feat, z_dte, x_dte)
        ...
        for i, blk in enumerate(self.blocks):
            ...
            '''
            if i >= 1:
                if self.prompt_type in ['vipt_deep_no_filter']:
                    x_ori = x
                    ...
                    # x_f = torch.cat((z_tokens,x_tokens),dim=1)
                    z_dte = self.prompt_norms[i](z_dte)
                    x_dte = self.prompt_norms[i](x_dte)
                    prompt_emb, z_dte, x_dte = self.prompt_blocks[i](x, z_dte, x_dte)

                    # x = combine_tokens(z, x, mode=self.cat_mode)
                    # re-conduct CE
                    prompt_emb = candidate_elimination_prompt(prompt_emb, global_index_t.shape[1], global_index_s)
                    x= x_ori

            x, global_index_t, global_index_s, removed_index_s, attn = \
                blk(x, global_index_t, global_index_s,prompt_emb, mask_x, ce_template_mask, ce_keep_rate)
