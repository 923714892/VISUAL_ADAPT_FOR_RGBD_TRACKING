    ...
    def __init__(self, smooth=False , b = None):
        ...
        if smooth:
            if b == 5 :
                ...
                self.smooth = nn.Parameter(torch.zeros(1) + 2.0)
            else:
                print(".................wrong......................")
...
class Prompt_block(nn.Module):
    def __init__(self, inplanes=None, hide_channel=None, smooth=False):
        ...
        self.fovea_0 = Fovea(smooth=smooth, b = 5)
        self.fovea_1 = Fovea(smooth = smooth, b = 2)
        self.untied_template_pos_enc_p = Untied2DPositionalEncoder(dim=hide_channel, num_heads=hide_channel, h=8, w=8, with_q=False)
        ...
        self.cross_attention_p = CrossAttentionBlock(dim=hide_channel,num_heads=hide_channel)
    ...
    def forward(self, RGB_feature_s, RGB_feature_t,D_feature_s,D_feature_t ):
        ...
        untied_template_pos_enc_k = self.untied_template_pos_enc_p()
        untied_search_pos_enc_p_q ,untied_search_pos_enc_p_k = self.untied_search_pos_enc_p()
        attn_pos_enc_p = (untied_search_pos_enc_p_q @ torch.cat((untied_template_pos_enc_k,untied_search_pos_enc_p_k),dim=1).transpose(-2, -1)).unsqueeze(0)
        attn_pos_enc_p = attn_pos_enc_p + self.rpe_bias_table_p(self.rpe_index_p)
        x1 = feature2token(x1)
        x2 = feature2token(x2)
        prompt_cross = self.cross_attention_p(q=x1, kv=torch.cat((x2,x1),dim=1), q_ape=None, k_ape=None,
                                                attn_pos=attn_pos_enc_p)
        # prompt_cross = self.sparse_attention_a(prompt_cross)
        prompt_cross = token2feature(prompt_cross)
        x1 = token2feature(x1)
        x2 = token2feature(x2)
        x = self.fovea_1(x0) + prompt_cross
        x = self.drop_out(x)
        x = self.conv1x1(x)
